...
async function handlePriceCommand(phoneNumber, symbol) {
  try {
    const resolveHermesSymbol = (s) => {
      const u = (s || '').toUpperCase();
      if (u === 'ETH' || u === 'ETHUSD' || u === 'ETH/USD') return 'ETH/USD';
      if (u === 'BTC' || u === 'BTCUSD' || u === 'BTC/USD') return 'BTC/USD';
      return u; // allow custom symbols if provided correctly
    };
    const pair = resolveHermesSymbol(symbol);

    const symbolKey = pair.replace('/', '') // ETH/USD -> ETHUSD
                         .toUpperCase();

    const envId = (config.pyth.priceIds && config.pyth.priceIds[symbolKey]) || null;

    const fetchById = async (id) => {
      const url = `${config.pyth.hermesHttp}/v2/updates/price/latest?ids[]=${encodeURIComponent(id)}&parsed=true&ignore_invalid_price_ids=true`;
      const resp = await axios.get(url);
      const parsed = resp.data && resp.data.parsed;
      const item = Array.isArray(parsed) ? parsed[0] : null;
      if (!item || !item.price) return null;
      const base = Number(item.price.price);
      const expo = Number(item.price.expo || 0);
      if (!isFinite(base) || !isFinite(expo)) return null;
      return base * Math.pow(10, expo);
    };

    const tryDiscoverId = async () => {
      // Best-effort discovery if env not set
      const query = symbolKey.startsWith('BTC') ? 'bitcoin' : (symbolKey.startsWith('ETH') ? 'eth' : symbolKey);
      const url = `${config.pyth.hermesHttp}/v2/price_feeds?query=${encodeURIComponent(query)}&asset_type=crypto`;
      const resp = await axios.get(url);
      const feeds = Array.isArray(resp.data) ? resp.data : [];
      const isMatch = (attr) => {
        const ds = (attr.display_symbol || '').toUpperCase();
        const gs = (attr.generic_symbol || '').toUpperCase();
        const base = (attr.base || '').toUpperCase();
        if (symbolKey === 'BTCUSD') return ds === 'BITCOIN/USD' || gs === 'BTCUSD' || base === 'BTC' || base === 'BITCOIN';
        if (symbolKey === 'ETHUSD') return ds === 'ETH/USD' || gs === 'ETHUSD' || base === 'ETH' || base === 'ETHEREUM';
        return ds.replace('/', '') === symbolKey || gs === symbolKey;
      };
      const found = feeds.find((f) => f && f.attributes && isMatch(f.attributes));
      return found ? found.id : null;
    };

    let priceNum = null;
    if (envId) {
      priceNum = await fetchById(envId);
    } else {
      const discoveredId = await tryDiscoverId();
      if (discoveredId) priceNum = await fetchById(discoveredId);
    }

    if (priceNum === null) {
      // Fallback: lightweight latest by symbol (may require exact symbol name on Hermes)
      const url = `${config.pyth.hermesHttp}/v2/price/latest?symbols=${encodeURIComponent(pair)}`;
      const resp = await axios.get(url);
      const data = resp.data;
      const entry = Array.isArray(data) ? data[0] : data;
      if (entry && entry.price && (entry.price.price !== undefined)) {
        const base = Number(entry.price.price);
        const expo = Number(entry.price.expo || 0);
        priceNum = base * Math.pow(10, expo);
      } else if (entry && typeof entry.price === 'number') {
        priceNum = entry.price;
      }
    }

    if (priceNum === null || isNaN(priceNum)) {
      await smsService.sendErrorMessage(phoneNumber, `Price not available for ${pair}. Configure PYTH_${symbolKey}_ID in .env for reliability.`);
      return;
    }
    const formatted = priceNum >= 1 ? priceNum.toFixed(2) : priceNum.toPrecision(4);
    await smsService.sendSms(phoneNumber, `Price ${pair}: ${formatted}`);
  } catch (e) {
    await smsService.sendErrorMessage(phoneNumber, `Failed to fetch price for ${symbol}`);
  }
}